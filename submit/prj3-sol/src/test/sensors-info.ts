...
import { DEFAULT_COUNT, DEFAULT_INDEX } from '../lib/params.js';
...
describe('sensors', () => {
  ...
  describe('sensor reading add validity checks', () => {
    ...
    it('must error EXISTS on duplicate sensor reading', async () => {
      ...
      const addResult1 = await sensorsInfo.addSensorReading(reading);
      assert(addResult1.isOk === true);
      const search =
	{ sensorId: reading.sensorId, timestamp: reading.timestamp };
      const findResult1 = await sensorsInfo.findSensorReadings(search);
      assert(findResult1.isOk === true);
      expect(findResult1.val[0].value).to.equal(Number(value));
      const reading1 = { ...reading, value: value1 };
      const addResult2 = await sensorsInfo.addSensorReading(reading1);
      assert(addResult2.isOk === false);
      expect(addResult2.errors[0].options.code).to.equal('EXISTS');
    });
  ...
  describe('sensor-types search', () => {
    ...
    it('must find all sensor types', async () => {
      const findResult = await sensorsInfo.findSensorTypes(FIND_ALL);
    ...
    it('must find all sensor types for a particular quantity', async () => {
      ...
      const findResult =
	await sensorsInfo.findSensorTypes(query);
      assert(findResult.isOk === true);
    ...
    it('must find first page of sensor types', async () => {
      ...
      assert(findResult.isOk === true);
      expect(findResult.val).to.have.length(DEFAULT_COUNT);
    });
    ...
    it('must find next three results after first page of results', async () => {
      ...
      const findResult = await sensorsInfo.findSensorTypes(query);
      assert(findResult.isOk === true);
      expect(findResult.val).to.have.length(nResults);
      const expected = sensorTypes.sort((a, b) => a.id.localeCompare(b.id))
	...
	.map(req => {
      ...
      expect(findResult.val).to.deep.equal(expected);
    });        
  ...
  describe('sensors search', () => {
    ...
    it('must find all sensors', async () => {
      const findResult = await sensorsInfo.findSensors(FIND_ALL);
      assert(findResult.isOk === true);
    ...
    it('must find all sensors having a particular sensorTypeId', async () => {
      ...
      const findResult = await sensorsInfo.findSensors(query);
      assert(findResult.isOk === true);
	...
	sensors.map(s => s.sensorTypeId).find(id => id !== sensorTypeId)!;
      const findResult =
	await sensorsInfo.findSensors({id, sensorTypeId: sOther});
      assert(findResult.isOk === true);
      expect(findResult.val).to.have.length(0);
    });
  ...
  describe('sensor readings search', () => {
    ...
    it('must find all sensor readings for a particular sensor', async () => {
      ...
      const findResult = await sensorsInfo.findSensorReadings(query);
      assert(findResult.isOk === true);
	...
	String(Number(minTimestamp) + (n-1) * Number(period));
      const search = { ...FIND_ALL, sensorId, minTimestamp, maxTimestamp };
      const findResult = await sensorsInfo.findSensorReadings(search);
      assert(findResult.isOk === true);
    ...
    it('must find all sensor readings having a particular value', async () => {
      ...
      const search = { ...FIND_ALL, sensorId, minValue, maxValue };
      const findResult = await sensorsInfo.findSensorReadings(search);
      assert(findResult.isOk === true);
       ...
       async () =>
    {
      ...
      const search = { ...FIND_ALL, sensorId, minValue };
      const findResult = await sensorsInfo.findSensorReadings(search);
      assert(findResult.isOk === true);
